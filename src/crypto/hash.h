...
#define CN_STANDARD_MEMORY              2097152 // 2MB scratchpad 2^21
#define CN_STANDARD_ITER                1048576 // 2^20

// Cryptonight Lite Definitions
#define CN_LITE_MEMORY                  1048576 // 1MB scratchpad 2^20
#define CN_LITE_ITER                    524288 // 2^19

// Cryptonight Heavy Definitions
#define CN_HEAVY_MEMORY                 4194304 // 4MB scratchpad
#define CN_HEAVY_ITER                   262144
...
#define CN_SOFT_SHELL_WINDOW            1024 // This defines how many blocks we cycle through as part of our algo sine wave
...
#include "hash-ops.h"
  ...
  */
  ...
  // void cn_slow_hash(const void *data, size_t length, char *hash, int light, int variant, int prehashed, uint32_t scratchpad, uint32_t iterations, uint32_t forceMemory)
  inline void cn_slow_hash_v6(const void *data, size_t length, Hash &hash) {
    cn_slow_hash(data, length, reinterpret_cast<char *>(&hash), 0, 0, 0, CN_STANDARD_MEMORY, CN_STANDARD_ITER);
  }

  inline void cn_slow_hash_v7(const void *data, size_t length, Hash &hash) {
    cn_slow_hash(data, length, reinterpret_cast<char *>(&hash), 0, 1, 0, CN_STANDARD_MEMORY, CN_STANDARD_ITER);
  }
  ...
  inline void cn_lite_slow_hash_v0_correct(const void *data, size_t length, Hash &hash) {
    cn_slow_hash(data, length, reinterpret_cast<char *>(&hash), 1, 0, 0, CN_LITE_MEMORY, CN_LITE_ITER);
  }

  inline void cn_lite_slow_hash_v1_correct(const void *data, size_t length, Hash &hash) {
    cn_slow_hash(data, length, reinterpret_cast<char *>(&hash), 1, 1, 0, CN_LITE_MEMORY, CN_LITE_ITER);
  }

  inline void cn_soft_shell(const void *data, size_t length, Hash &hash, uint32_t height) {
